...
def reorder(file_name, group = int(3), increment = int(3), max_pos_angle = int(54), min_neg_angle = int(-54), scheme = "pos"):
    #path = os.getcwd()
    #for filename in os.listdir(path):
        if file_name.endswith(".tlt"):
           tiltname = file_name.split(".")[0]
           extension = ".order"
           ordername = tiltname + extension
           tiltfile = open(file_name, 'r').read().splitlines()
           ...
           reorderfile.close()
            ...
            print("Please provide tile file with extension .tlt")
